...
- [ ] Server par `public/hot` naam ki file **nahi** honi chahiye (ye sirf aapke computer par `npm run dev` ke liye hai). Agar hai to delete karein, warna site bina CSS ke khulegi. **26 Sep 2026 ko yahi hua:** file upload ke saath `hot` bhi chali gayi, aur live site ka HTML `http://[::1]:5173/...` (visitor ka apna computer) se CSS/JS maangne laga, jo kisi ke paas nahi hota. **27 Sep 2026 se `npm run build` khud `public/hot` hata deta hai** (`tools/remove-hot.mjs`), aur `npm run dev` use dobara bana deta hai. Isliye upload ka tareeka: pehle `npm run build` chalayein, phir `public/build` upload karein (build ke baad `hot` hoti hi nahi). Agar dev chalte hue build chalaya to dev dobara chalana padega. Phir bhi upload me `public/hot` ko bahar rakhein; `hot` git me nahi jaati, isliye `git pull` se ye galti nahi hoti.
