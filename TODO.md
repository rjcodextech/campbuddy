...
- [x] **Sync (27 Sep, ho gaya, commit me):** Explore ka "I met them" / Met / Couldn't / Undo / Try again ab My Day ke saath ek hi record se. Roster (`r:`) aur discovery (`d:`) ek insaan ke do record hain, wo abhi baaki (roster id chahiye).
- [x] **My schedule ke status chips (27 Sep, ho gaya, commit me):** All / To do / Done / Couldn't / Hidden, sessions aur logon dono par, chip ka number = card ki ginti, khali chips chhupte hain, "Hide done" ki jagah. Hidden ka Show again bhi.
- [x] **✕ Hide (27 Sep, ho gaya, commit me):** card ke corner me ✕, "Hidden (N)" me Show again ke saath (Explore aur My Day dono me), Remove ki jagah "Hide from plan" (kuch delete nahi), mutual wave par confirm, Home ke count se hataye hue bahar.
- [x] **Attendee list ↔ discovery card ek insaan (27 Sep, ho gaya, commit me):** card me `roster_id` (PHP ek field), `r:<id>` ek record, purane `d:` record padhe jate hain aur `mergedInto` se ek me mil jate hain, kuch delete nahi.
- [x] **Ek action, ek naam (27 Sep, ho gaya, commit me):** "I met them" ab "✓ Met" (My Day jaisa), "✗ Couldn't" ab "✗ Couldn't meet", Explore me "Try again" ab "Undo", "Hide from plan" ab "Hide", ek jaisa toast. Ek hi list (`LABELS`) aur uska test.
- [x] **Do tab ek saath (27 Sep, ho gaya, commit me):** ek tab me Met / Hide / Save karo to doosra tab (ya installed app) apne aap badalta hai: Explore, My schedule, Home ka count. BroadcastChannel se, kuch personal nahi jata.
- [x] **Session ★ un-save (26 Sep, owner ka faisla: jaisa hai waisa):** ★ dobara dabane par session, uska reminder aur Done/Couldn't tick delete hote hain (sessions par note nahi hota) (log wale ✕ Hide jaisa nahi). Dobara ★ karke save ho sakta hai: Full schedule se, My schedule ke card ke baad Full schedule se, aur khule session ke "Save to My Day" button se; reload ke baad bhi. Asli Chrome me jaancha, do naye test bhi (server ka delete-phir-save, aur offline off-phir-on). Save dobara karne par tick khali se shuru hota hai. Agar event me shikayat aaye to Hide jaisa (record rakhna) banaya ja sakta hai.
...
| 21 | ~~`fetch_log` kabhi saaf nahi hota~~ — **ho gaya (local `dev`, upload baaki):** roz raat 03:20 par 7 din se purani rows hatti hain, har (event, job) ki sabse nayi row bachti hai (`php artisan campbuddy:prune-fetch-log`, `--dry-run` sirf ginti). Ek saal ke live-jaise log (2.7 lakh rows) par pehli safai ~11 s, phir roz ~5 ms; Errors 290→95 ms, Dashboard 374→94 ms | — | Deploy ke baad koi kaam nahi (scheduler chalata hai); chahein to ek baar haath se chalayein |
| 22 | ~~Checklist item *hataane* par khule apps ko khabar nahi jaati~~ — **ho gaya (local `dev`, upload baaki):** `DataVersion` me deals aur quests ki ginti judi hai; jodne, badalne aur hataane teeno par version badalta hai | — | **Deploy ka dhyan:** fingerprint badalta hai, isliye deploy ke baad har khula app ek baar reload hoga — event ke beech peak par deploy na karein |
