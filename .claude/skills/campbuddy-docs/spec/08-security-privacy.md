...
> **Current implementation note:** no analytics/`track()` wrapper actually exists in the codebase yet (GA4 isn't wired up) — this guardrail is still the intended policy for whenever one is added, and the [Deals lead-capture](03-functional-requirements/07-deals.md) code carries an explicit comment not to feed captured Name/Email/Mobile into any future tracking call.
