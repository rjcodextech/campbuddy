...
>
> **Picker:** each card's avatar is the event's favicon, else its logo, else CampBuddy's icon, and falls back to CampBuddy's icon if the image fails to load. The "up to 5 events" cap is unchanged — a 6th active event is reachable by URL but not listed.
