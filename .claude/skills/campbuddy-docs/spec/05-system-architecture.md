...
>
> **Auto-publish / auto-archive (added post-launch, not in the original plan at all):** `EvaluateEventLifecycleJob` runs daily. A draft event is promoted straight to `active` once its own website responds successfully *and* the roster scrape ([3.3](03-functional-requirements/03-roster-ingestion.md)) finds at least one real, non-suppressed attendee — trusted as a signal the event is real and far enough along, without needing a human click. Separately, any `active`/`approved` event is auto-archived once its **real last day** (end date, start date or last scheduled session day — whichever is latest) plus **3 retention days** have passed at the venue ([23](23-data-retention.md)); a draft that's already past and never got promoted is just left alone (never archived, since it was never live).
...
> **Current implementation note — markup lives in Blade, not JS:** the attendee JS (`resources/js/attendee/*`) never authors HTML strings. Anything it has to render at runtime from local/IndexedDB or API data (roster rows, schedule items, quest cards, dialogs, toasts…) is a `<template id="tpl-…">` in a Blade partial under `resources/views/attendee/templates/` — `shared.blade.php` (toast + iOS install dialogs, included by both `layouts/attendee.blade.php` and `welcome.blade.php`), plus one partial per screen (`home`, `my-day`, `quest`, `camp-card`, `contribute`, `discovery`, `people`, `explore`), each `@include`d by the page that needs it. Static-only chunks (the onboarding form, the "Your data" card, the People shell) are plain Blade instead, and JS only wires them. `resources/js/attendee/template.js` is the whole client-side "engine": `render(id, slots)` / `renderFragment(id, slots)` clone a template and fill its `data-slot="…"` elements (text, attributes, classes, children; `null`/`false` removes an optional element; a `<slot data-slot>` is an unwrapped placeholder for lists). Because values only ever go in as text/attributes, nothing is parsed as markup — no hand-rolled escaping, and third-party text (roster names, speaker bios) can't inject HTML. The templates ship inside the server-rendered page, so the service worker caches them with it and rendering still works offline. To change what something looks like, edit its Blade template; JS only changes when the *data* or *behaviour* does.
