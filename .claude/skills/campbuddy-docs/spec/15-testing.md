...
- **Long match lists (Sept 2026):** `tests/js/list-window.test.mjs` covers the "first 3, then Show 10 more" logic ([3.4](03-functional-requirements/04-matching.md)); it was also checked in a real Chrome at 390 px against a throwaway SQLite copy of the app seeded with 40 discovery profiles (3 cards shown, the number on the button is exactly how many a tap opens — checked on every tap to the end of the list — no card is repeated or added, Wave / I met them / + Meet work on a card that had been hidden, and the list stays open after the re-render).
- **People status (Sept 2026):** `people-state.test.mjs` (the states, groups, chip counts), `people-status.test.mjs` (one record written the same way from every screen, nothing deleted, hide/show again restores what it was), `plan.test.mjs` (what counts in "N of M"). Checked in real Chrome at 390 px on a throwaway SQLite copy with 40 seeded profiles and a small attendee list: Explore ↔ My Day both ways for I met them / Met / Couldn't / Try again / Undo, persistence after a reload, redraw on returning to the page, and the attendee-list button following the same record (24 checks).
- **Status chips (Sept 2026):** the chip logic is in `people-state.test.mjs` (groups, counts that add up, which chips show, falling back to "All", starting chip); the page was checked in real Chrome at 390 px (27 checks): the numbers on the chips against the cards shown for every chip, Met / Couldn't moving people between chips, progress ("N of M") excluding unplanned and hidden people, hide → Hidden view → Show again giving back exactly what it was with the note, attended / missed sessions in the same chips, the chosen chip surviving a reload, and the old *Hide done* becoming "To do". (It also caught a real bug: the hidden card was picked up by the normal cards' wiring and threw.)
- **✕ Hide (Sept 2026):** real Chrome at 390 px (28 checks): a ✕ on every match; hiding never-planned and planned people (the note kept, "planned" given back by Show again, on Explore and on My Day); the Meet sheet's "Hide from plan" taking the card off at once; the confirmation for a mutual-wave match (no keeps them, yes hides); Home's "wants to meet you" count going down by one; the Hidden section surviving a reload; and the number of records never going down. `startingFilter` never starts on Hidden (`people-state.test.mjs`). Note for the next person: the fake profiles must have unique names, or "who" checks in these scripts are ambiguous.
- **Attendee list ↔ match (Sept 2026):** `DiscoveryIdentityTest` checks `roster_id` (the entry's id when picked from the list, `null` otherwise); `people-state` / `people-status` / `plan` tests cover the keys, an older `d:` record being folded in (copied, marked, never deleted) and a person listed once; real Chrome at 390 px (15 checks) with three profiles that each claimed a list entry: "+ Meet" on the list → the card follows at once, Met on the card → the list row follows and My Day has one entry, Met in My Day → the card and the row follow, hide → the row goes back to "+ Meet", and a record made by the previous version is read and folded in.
- **Wording (Sept 2026):** `tests/js/wording.test.mjs` fails if the Blade templates (the match card's Met button and label, My schedule's Met / Couldn't meet buttons) stop agreeing with `LABELS`, or if a screen goes back to spelling out an older name ("Try again", "Hide from plan", …).
- **Two open windows (Sept 2026):** `people-sync.test.mjs` (a second copy of the module plays the other window: the sender isn't told, everyone else is, unsubscribing works, messages of other kinds are ignored, no BroadcastChannel = nothing happens) and `people-status.test.mjs` (told once after a change, never for a no-op or a lookup); real Chrome at 390 px (11 checks): a Met, a Hide and a Save each send one message, and Explore, My schedule and Home's count follow a change written by "another window" with no focus change.
