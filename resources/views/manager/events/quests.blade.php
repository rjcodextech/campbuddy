<x-manager-layout title="Quests & checklist" :subtitle="$event->display_name"
                  :breadcrumbs="[['My events', route('manager.dashboard')], [$event->display_name, route('manager.events.details', $event)], ['Quests & checklist']]">
    <x-manager.event-nav :event="$event" current="quests" />
    ...
    <div class="max-w-4xl space-y-6">
                    ...
                    @foreach ($quests as $quest)
                        <li @class(['px-5 py-3 sm:px-6', 'bg-paper/60 text-muted' => ! $quest->is_active])>
                            <div class="flex flex-col gap-2 md:flex-row md:items-center md:gap-3">
                                <form method="POST" action="{{ route('manager.events.quests.update', [$event, $quest->id]) }}"
                                ...
                                <x-action-form :action="route('manager.events.quests.destroy', [$event, $quest->id])" method="DELETE"
        ...
        <form method="POST" action="{{ route('manager.events.quests.store', $event) }}">
            @csrf
